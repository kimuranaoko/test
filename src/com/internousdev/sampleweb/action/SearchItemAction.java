...
public class SearchItemAction extends ActionSupport implements SessionAware{
	...
	public String execute(){
		...
		switch(categoryId){
			case "1":
//				カテゴリーID１→全部だったら該当の全件、他だったらカテゴリーしぼってから該当のもの全件
	//			replace→全角スペースは半角スペースに直す　split→半角スペースで分割し、それぞれを配列の要素として格納
...
//				IndexOutOfBoundsException：ある種のインデックス (配列、文字列、ベクトルなど) が範囲外であることを示すためにスローされます
		...
		session.put("keywordErrorMessageList",keywordsErrorMessageList);

//		もしカテゴリーリストが空ならいれてあげる（多分いきなり検索するときもあるから）
...
//		検索結果がNULLじゃなければページ数を先にputしておいてるようです
