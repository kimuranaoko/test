...
drop database if exists takaraduka;
create database if not exists takaraduka;
...
id int primary key not null auto_increment comment "ID",
ticket_id int unique not null comment "チケットID",
ticket_name varchar(100) unique not null comment "公演名",
ticket_name_kana varchar(100) not null comment "公演名かな",
ticket_description varchar(255) not null comment "公演詳細",
troupe_id int not null not null comment "組ID",
...
ticket_count int not null comment "枚数",
...
foreign key(ticket_id) references product_info(ticket_id)
)
default charset=utf8
comment="購入履歴情報テーブル"
;
...
troupe_id int not null unique comment "組ID",
troupe_name varchar(20) not null unique comment "組名",
troupe_description varchar(100) comment "組詳細",
...
(1,1,"花組","花組の説明",now(),null,
(2,2,"月組","月組の説明",now(),null,
(3,3,"雪組","雪組の説明",now(),null,
(4,4,"星組","星組の説明",now(),null,
(5,5,"宙組","宙組の説明",now(),null,
(6,6,"専科","専科の説明",now(),null);

create table hanagata_history_info(
id int primary key not null comment "ID",
hanagata_id int not null unique comment "華形ID",
program_name varchar(100) unique not null comment "公演名",
program_name_kana varchar(100) not null comment "公演名かな",
program_description varchar(255) not null comment "公演詳細",
image_file_path varchar(100) comment "画像ファイルパス",
image_file_name varchar(50) comment "画像ファイル名",
